...
def max_block_size(p: int) -> int:
    """
    Karena kita akan mengubah blok bytes menjadi integer < p,
    maka block_size_bytes = floor((bit_length(p) - 1) / 8).
    """
    bit_len = p.bit_length()
    return (bit_len - 1) // 8  # floor((bit_len(p) - 1)/8)


# ----------------------------------------------------------------------
# 3. Fungsi pembantu: baca file JS dan bagi menjadi blok integer
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 5. Saat dijalankan sebagai script utama di GitHub Action
# ----------------------------------------------------------------------
if __name__ == "__main__":
    # Asumsikan hello.js dan encrypt_js_for_html.py ada di root repo
    repo_root = os.getcwd()  # di GitHub Actions, ini adalah root repo setelah checkout
    js_path = os.path.join(repo_root, "hello.js")
    output_json = os.path.join(repo_root, "hello_ciphertext.json")
